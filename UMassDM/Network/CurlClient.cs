...
namespace UMassDM.Network
{
    ...
    public class CurlClient
    {
        ...
        public async Task<CurlResponse> PutDiscord(string endpoint, string auth = null, bool xsuper = false, string headers = null, string json = null)
        {
            return await Put(string.Format("https://discord.com/api/v{0}{1}", Config.Instance.APIVersion, endpoint), auth, xsuper, headers, json);
        }

        public async Task<CurlResponse> Put(string endpoint, string auth = null, bool xsuper = false, string headers = null, string json = null)
        {
            if (!m_initialized)
                return new CurlResponse
                {
            ...
            using (SafeEasyHandle easy = GetEasy(endpoint, auth, xsuper, headers, out headerlist))
            {
                ...
                CurlNative.Easy.SetOpt(easy, CURLoption.PUT, 1);
        ...
        private SafeEasyHandle GetEasy(string url, string auth, bool xsuper, string headers, out SafeSlistHandle headerlist)
        {
            ...
            if (m_ssluse)
            {
                ...
                if (m_ja3 != null)
                {
                    ...
                    //CurlNative.Easy.SetOpt(easy, CURLoption.SSL_CIPHER_LIST, "TLS_AES_256_GCM_SHA384");
                }
            }
